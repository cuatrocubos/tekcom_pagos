  ...
  for purchase_order in purchase_orders:
    ...
    if len(solicitante_details) > 0:
      ...
      if purchase_order.custom_solicitado_por == None:
        frappe.db.set_value('Purchase Order', purchase_order.name, 'custom_solicitado_por', solicitante_email)
      ...
      if len(aprobador_details) > 0:
        ...
        frappe.db.set_value('Purchase Order', purchase_order.name, 'custom_aprobado_por', aprobador_email)
        frappe.db.set_value('Purchase Order', purchase_order.name, 'custom_fecha_hora_aprobacion', fecha_aprobacion)
